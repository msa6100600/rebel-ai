...
const cloudCategory = { "تفضيل": "preference", "حقيقة": "profile", "سياق": "temporary", "استنتاج": "decision" } as const;

export default function ApprovalsScreen() {
  const { approvals, approve, reject } = useRebelStore();
  const createMemory = trpc.cloud.memories.create.useMutation();
  const pending = approvals.filter((approval) => approval.status === "بانتظار قرارك").length;
  const approveAndSave = async (item: ApprovalItem) => {
    try {
      if (item.memory) await createMemory.mutateAsync({ category: cloudCategory[item.memory.category], title: item.memory.title, content: item.memory.content });
      approve(item.id);
      haptic.success();
    } catch { Alert.alert("تعذر الحفظ", "لم تُحفظ المعلومة في الذاكرة السحابية. بقي الاقتراح بانتظار موافقتك ويمكنك إعادة المحاولة."); haptic.warning(); }
  };
  const renderApproval = ({ item }: { item: ApprovalItem }) => { const pendingItem = item.status === "بانتظار قرارك"; const tone = item.status === "تمت الموافقة" ? "success" : item.status === "تم الرفض" ? "neutral" : "warning"; return <View style={styles.card}><View style={styles.top}><StatusPill label={item.status} tone={tone} /><Text style={styles.type}>{item.type}</Text></View><Text style={styles.title}>{item.title}</Text><Text style={styles.detail}>{item.detail}</Text>{pendingItem ? <View style={styles.actions}><Pressable accessibilityRole="button" onPress={() => { reject(item.id); haptic.medium(); }} style={({ pressed }) => [styles.reject, pressed && styles.pressed]}><Text style={styles.rejectText}>رفض</Text></Pressable><Pressable accessibilityRole="button" onPress={() => approveAndSave(item)} disabled={createMemory.isPending} style={({ pressed }) => [styles.approve, createMemory.isPending && styles.disabled, pressed && styles.pressed]}><IconSymbol name="checkmark.seal.fill" color="#FFFFFF" size={17} /><Text style={styles.approveText}>{createMemory.isPending ? "جارٍ الحفظ…" : "موافقة وحفظ"}</Text></Pressable></View> : null}</View>; };
  return (
