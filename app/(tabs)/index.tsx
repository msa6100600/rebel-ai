...
import { rebelGpts, voiceProfiles, useRebelStore, type ChatMessage } from "@/lib/rebel-store";
...
export default function ChatScreen() {
  ...
  const sendMessage = useCallback(async (override?: string) => {
    ...
    try {
      const result = await chat.mutateAsync({ message: text, memories: memories.slice(0, 8).map(({ title, content, category }) => ({ title, content, category })), language: selectedVoice.language, model: preferences.selectedModel as "gpt-5" | "gpt-5-mini" | "claude-sonnet-4-6" | "gemini-3.1-pro-preview", gptId: activeGpt.id });
