...
export default function ChatScreen() {
  ...
  const sendMessage = useCallback(async (override?: string) => {
    ...
    try {
      const result = await chat.mutateAsync({ message: text, conversationId: preferences.temporaryChat ? undefined : activeConversationId, projectId: preferences.temporaryChat ? undefined : preferences.activeProjectId, temporary: preferences.temporaryChat, memories: [], language: selectedVoice.language, model: preferences.selectedModel, gptId: activeGpt.id });
  ...
  }, [activeConversationId, activeGpt.id, addMessage, chat, conversations, draft, preferences.activeProjectId, preferences.selectedModel, preferences.temporaryChat, selectedVoice.language]);
  ...
  const toggleRecording = useCallback(async () => {
    ...
    try {
      if (recorderState.isRecording) {
        ...
        const result = await transcription.mutateAsync({ audioBase64: await audioFile.base64(), mimeType: getMimeType(), language: selectedVoice.language });
