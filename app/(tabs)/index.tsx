...
const taskCards = [
  { title: "اكتب بوضوح", detail: "رسالة أو محتوى", prompt: "ساعدني في كتابة نص واضح ومناسب لهذا الهدف: " },
  { title: "حلّل وقارن", detail: "خيارات وقرار", prompt: "حلّل الخيارات التالية، وقارن بينها بوضوح مع افتراضات وحدود كل خيار: " },
  { title: "خطّة عملية", detail: "خطوات قابلة للتنفيذ", prompt: "حوّل هذا الهدف إلى خطة عملية قصيرة بخطوات واضحة: " },
  { title: "اشرح وبسّط", detail: "تعلم وفهم", prompt: "اشرح هذا الموضوع ببساطة، ثم أعطني مثالاً وخطوة تالية: " },
] as const;
...
export default function ChatScreen() {
  ...
  const sendMessage = useCallback(async (override?: string) => {
    ...
    try {
      ...
      appendMessage({ role: "assistant", text: result.answer, insight: result.insight, confidence: result.confidence, model: result.model });
  ...
  const renderMessage = ({ item }: { item: ChatMessage }) => {
    ...
    return <View style={[styles.messageRow, item.role === "user" ? styles.userRow : styles.assistantRow]}><View style={[styles.message, item.role === "user" ? styles.userMessage : styles.assistantMessage, item.isError && styles.errorMessage]}>{item.role === "assistant" ? <View style={styles.assistantTop}><View style={[styles.assistantMark, { backgroundColor: activeGpt.color }]}><Text style={styles.assistantMarkText}>{activeGpt.shortName.slice(0, 1)}</Text></View><Text style={styles.role}>{activeGpt.name}</Text><View style={styles.modelBadge}><Text style={styles.modelBadgeText}>{messageModel?.shortName}</Text></View><Pressable accessibilityRole="button" accessibilityLabel="قراءة الرد بصوت" onPress={() => speak(item.text)} style={({ pressed }) => [styles.soundButton, pressed && styles.pressed]}><IconSymbol name="speaker.wave.2.fill" size={17} color="#65656E" /></Pressable></View> : null}<Text style={[styles.messageText, item.role === "user" && styles.userMessageText]}>{item.text}</Text>{item.insight ? <Text style={styles.insightText}>{item.insight}</Text> : null}{item.role === "assistant" && !item.isError ? <View style={styles.messageActions}><Pressable accessibilityRole="button" onPress={() => copyResponse(item.text)} style={({ pressed }) => [styles.messageAction, pressed && styles.pressed]}><Text style={styles.messageActionText}>نسخ</Text></Pressable><Pressable accessibilityRole="button" onPress={() => shareResponse(item.text)} style={({ pressed }) => [styles.messageAction, pressed && styles.pressed]}><Text style={styles.messageActionText}>مشاركة</Text></Pressable><Pressable accessibilityRole="button" onPress={() => rewriteResponse(item.text, "مبسطة")} style={({ pressed }) => [styles.messageAction, pressed && styles.pressed]}><Text style={styles.messageActionText}>تبسيط</Text></Pressable><Pressable accessibilityRole="button" onPress={() => rewriteResponse(item.text, "مصرية")} style={({ pressed }) => [styles.messageAction, pressed && styles.pressed]}><Text style={styles.messageActionText}>مصرية</Text></Pressable><Pressable accessibilityRole="button" onPress={() => rewriteResponse(item.text, "رسمية")} style={({ pressed }) => [styles.messageAction, pressed && styles.pressed]}><Text style={styles.messageActionText}>رسمية</Text></Pressable></View> : null}</View></View>;
  };
...
const styles = StyleSheet.create({
  taskGrid: { flexDirection: "row-reverse", flexWrap: "wrap", gap: 8, paddingHorizontal: 2, paddingBottom: 9 }, taskCard: { width: "48%", minHeight: 68, backgroundColor: "#FFFFFF", borderColor: "#E8E8EC", borderWidth: 1, borderRadius: 13, padding: 10, alignItems: "flex-end", justifyContent: "center", gap: 3 }, taskTitle: { color: "#2A2A30", fontSize: 13, fontWeight: "900", textAlign: "right" }, taskDetail: { color: "#7D7D86", fontSize: 10, textAlign: "right" }, toolDivider: { height: 1, backgroundColor: "#ECECEF", marginHorizontal: 6, marginBottom: 6 }, messageActions: { flexDirection: "row-reverse", flexWrap: "wrap", gap: 6, paddingTop: 4 }, messageAction: { borderRadius: 9, backgroundColor: "#F3F5F9", paddingHorizontal: 9, paddingVertical: 6 }, messageActionText: { color: "#4A5875", fontSize: 10, fontWeight: "800" },
