...
function SettingToggle({ value, onChange, title, detail }: { value: boolean; onChange: (value: boolean) => void; title: string; detail: string }) {
  return <View style={styles.settingLine}><Switch value={value} onValueChange={onChange} trackColor={{ false: "#2E3A5D", true: "#6950E8" }} thumbColor="#FFFFFF" /><View style={styles.settingText}><Text style={styles.settingTitle}>{title}</Text><Text style={styles.settingDetail}>{detail}</Text></View></View>;
}
